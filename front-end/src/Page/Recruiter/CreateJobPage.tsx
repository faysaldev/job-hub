...
const Badge = memo(
  ({
    ...
    <div
      className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider border ${className}`}
    >
      {children}
    </div>
  ),
  ...
  ({
    ...
    return (
      <div className="space-y-4">
        <label className="block text-sm font-bold text-[#234C6A]">
          {label}
        </label>
        ...
        {variant === "badge" ? (
          <div className="flex flex-wrap gap-2">
            {items?.map((item, index) => (
              <span
                key={`${item}-${index}`}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#234C6A]/10 text-[#234C6A] rounded-xl text-sm font-semibold border border-[#234C6A]/5"
              >
                {item}
                <button
                  type="button"
          ...
          <div className="space-y-2">
            {items?.map((item, index) => (
              ...
              >
                ...
                <span className="flex-1 text-sm font-medium text-[#234C6A]">
                  {item}
                </span>
                <button
                  type="button"
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          className="min-h-[250px] rounded-[2rem] border-[#E3E3E3] p-8 font-medium leading-relaxed resize-none"
        />
      </div>
    </div>
  );
      ...
      {STEPS.map((step) => {
        ...
        return (
          <div key={step.number} className="flex flex-col items-center">
            ...
            <span
              className={`mt-3 text-xs font-bold uppercase tracking-widest ${active ? "text-[#234C6A]" : "text-gray-300"}`}
            >
              {step.title}
            </span>
          </div>
      ...
      <div className="max-w-5xl mx-auto pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-3xl font-black text-[#234C6A] tracking-tight">
                Create Opportunity
              </h1>
        ...
        <FormProvider {...methods}>
          <form onSubmit={handleSubmit(onSubmit)}>
            <Card className="p-8 md:p-12 border-none bg-white shadow-2xl rounded-[2.5rem] relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-[#234C6A]/5 to-transparent rounded-full -mr-32 -mt-32 pointer-events-none" />
