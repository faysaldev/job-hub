...
const JobCard = memo(({ job }: { job: ApiJob }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className="job-card-item group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg hover:border-[#234C6A]/20 transition-all duration-200 cursor-pointer overflow-hidden"
    >
      {/* Top accent on hover */}
      <div className="h-0.5 w-full bg-gradient-to-r from-[#234C6A] to-[#456882] scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
      ...
      <div className="p-5 sm:p-6">
        <div className="flex gap-4 items-start">
          ...
          <div className="flex-shrink-0">
            {job.company?.companyLogo ? (
              ...
              <div className="w-12 h-12 rounded-xl bg-[#234C6A]/10 flex items-center justify-center">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <h3 className="text-base sm:text-lg font-bold text-[#234C6A] truncate group-hover:text-[#456882] transition-colors">
                  {job.title}
                </h3>
                <p className="text-sm font-medium text-[#456882] flex items-center gap-1.5 mt-0.5">
              ...
              {/* Arrow icon */}
              <ChevronRight className="h-5 w-5 text-gray-300 group-hover:text-[#234C6A] flex-shrink-0 mt-1 transition-colors" />
            </div>

            {/* Meta row */}
            <div className="flex flex-wrap items-center gap-2 mt-3">
              <span
                className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-lg ${TYPE_COLORS[job.type] || "bg-gray-100 text-gray-700"}`}
              >
                <Briefcase className="h-3 w-3" />
                {job.type.replace("-", " ")}
              </span>
              <span
                className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-lg ${LOCATION_COLORS[job.locationType] || "bg-gray-100 text-gray-700"}`}
              >
                <MapPin className="h-3 w-3" />
                {job.locationType}
              </span>
              <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-lg bg-[#234C6A]/5 text-[#234C6A]">
                <DollarSign className="h-3 w-3" />
                {formatSalary(job.salaryMin, job.salaryMax, job.salaryPeriod)}
              </span>
              <span className="inline-flex items-center gap-1 text-xs text-[#456882] px-2.5 py-1 rounded-lg bg-gray-50">
            ...
            {/* Location text */}
            <p className="text-xs text-[#456882]/80 mt-2 flex items-center gap-1">
            ...
            {job.skills?.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                {job.skills.slice(0, 4).map((skill) => (
                  <span
                    key={skill}
                    className="text-xs px-2 py-0.5 bg-[#234C6A]/5 text-[#234C6A] rounded-md font-medium border border-[#234C6A]/10"
                  >
                    {skill}
                  </span>
                ))}
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-50">
          <span className="text-xs text-[#456882]/60 flex items-center gap-1">
            <Clock className="h-3 w-3" />
            Posted {timeAgo(job.createdAt)}
          </span>
          <span
            className={`text-xs font-medium flex items-center gap-1 ${isExpiringSoon ? "text-red-500" : "text-[#456882]/60"}`}
          >
            <Clock className="h-3 w-3" />
...
const EmptyState = memo(() => (
  <Card className="p-12 text-center border-none bg-white shadow-md rounded-xl">
    ...
    <h3 className="text-xl font-bold text-[#234C6A] mb-2">No Jobs Found</h3>
...
//       router.push(queryString ? `/job?${queryString}` : "/job", {
//         scroll: false,
//       });
//     },
//     [searchParams, router],
...
//     // Update URL
//     const checkedSalaries = newSalaryRanges
//       .filter((s) => s.checked)
//       .map((s) => s.id);
//     updateURLParams({
...
//     const currentlyChecked = subcategoryFilters.find(
//       (s) => s.id === id,
//     )?.checked;
...
//     setSelectedSubcategory(null);
//     setCategoryFilters((prev) =>
//       prev.map((cat) => ({ ...cat, checked: false })),
//     );
//     setSubcategoryFilters([]);
//     // Update URL
//     updateURLParams({
...
//     setSelectedSubcategory(null);
//     setSubcategoryFilters((prev) =>
//       prev.map((sub) => ({ ...sub, checked: false })),
//     );
//     // Update URL
//     updateURLParams({
//       subcategory: null,
...
//     setExperienceLevels((prev) =>
//       prev.map((level) => ({ ...level, checked: true })),
//     );
//     setSalaryRanges((prev) =>
//       prev.map((range) => ({ ...range, checked: false })),
//     );
//     setCompanySizeFilters((prev) =>
//       prev.map((size) => ({ ...size, checked: false })),
//     );
//     setPostedDateFilters((prev) =>
//       prev.map((date) => ({ ...date, checked: false })),
//     );
//     setApplicantCountFilters((prev) =>
//       prev.map((count) => ({ ...count, checked: false })),
//     );
//     setCurrentPage(1);
